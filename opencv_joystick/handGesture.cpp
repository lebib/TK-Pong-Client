...
void HandGesture::printGestureInfo(Mat src){
	...
	putText(src,info,Point(ypos,xpos),fontFace,fontSize  ,fColor);
        xpos+=lineChange;
	info=string("Direction output: ") + string(intToString(getDirection()));
	putText(src,info,Point(ypos,xpos),fontFace,fontSize  ,fColor);
        xpos+=lineChange;
	info=string("Rectangle center x : ") + string(intToString(bRectCenter.x));
	putText(src,info,Point(ypos,xpos),fontFace,fontSize  ,fColor);
}
...
int HandGesture::getDirection(){
  ...
  if(bRectCenter.x < HandGesture::LEFT_LIMIT && bRectCenter.x > 0){
    return 1;
  }else if(bRectCenter.x < HandGesture::RIGHT_LIMIT){
    return 0;
  }else{
    return 2;
  }
